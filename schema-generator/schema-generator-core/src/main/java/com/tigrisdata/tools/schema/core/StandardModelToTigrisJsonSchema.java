...
public class StandardModelToTigrisJsonSchema implements ModelToJsonSchema {
  ...
  private static JsonNode customizeSchema(
    ...
    while (fields.hasNext()) {
      ...
      if (primaryKeysMap.isEmpty() && PROPERTIES.equals(field)) {
        // inject the _id property if user's schema misses pkeys
        ObjectNode propertiesObjectNode = jsonSchema.get(field).deepCopy();
        propertiesObjectNode.set(
            AutoGeneratedFields._ID.getFieldName(),
            new ObjectMapper().createObjectNode().put(TYPE, STRING).put(FORMAT, UUID));
        objectNode.set(field, propertiesObjectNode);
      } else {
    ...
    if (!primaryKeysMap.isEmpty()) {
      ...
      // add default pkey
      primaryKeys.add(AutoGeneratedFields._ID.getFieldName());
    }
