...
/**
 * This class is created referring {@code ConstructorRule} for TigrisDB
 * Schema This class is
 * responsible for creating constructors with PK as args. This is to tell
 * users those are required
 * field of your type.
 *
 * <p>NOTE: The order of the constructor argument matches the order defined
 * in the JSON schema
 */
public class TigrisDBConstructorRule implements Rule<JDefinedClass,
        JDefinedClass> {
    ...
    public TigrisDBConstructorRule(RuleFactory ruleFactory,
    ...
    private JDefinedClass handle(JsonNode node, JDefinedClass instanceClass,
                                 Schema currentSchema) {
        ...
        List<String> pkCombinedSuperPropertiesList =
                new ArrayList<>(pkCombinedSuperProperties);
        ...
        if (!pkPropertiesList.isEmpty() || !pkCombinedSuperPropertiesList.isEmpty()) {
            ...
            addFieldsConstructor(instanceClass, pkPropertiesList,
                    pkCombinedSuperPropertiesList);
        }
    ...
    private void addFieldsConstructor(
        ...
        JMethod instanceConstructor =
                generateFieldsConstructor(instanceClass, classProperties,
                        combinedSuperProperties);
        ...
        if (generationConfig.isGenerateBuilders() && generationConfig.isUseInnerClassBuilders()) {
            ...
            generateFieldsBuilderConstructor(
                    baseBuilderClass, concreteBuilderClass, instanceClass,
                    instanceConstructor);
        }
    }
    ...
    private LinkedHashSet<String> getConstructorProperties(JsonNode node) {
            ...
            if (pkArray.isArray()) {
                for (JsonNode pkEntry : pkArray) {
                    if (pkEntry.isTextual()) {
                        ...
                        throw new IllegalArgumentException(PRIMARY_KEYS +
                                "has to be of an array of string");
                    }
                }
            } else {
                throw new IllegalArgumentException(PRIMARY_KEYS + " has to be" +
                        " of " +
                        "array type");
            }
        }
        ...
        for (String draft4PKProperty : draft4PKProperties) {
            for (Iterator<Map.Entry<String, JsonNode>> properties = node.get(
                    "properties").fields();
                 properties.hasNext(); ) {
    ...
    /**
     * Recursive, walks the schema tree and assembles a list of all properties
     * of this schema's super
     * schemas
     */
    private LinkedHashSet<String> getSuperTypeConstructorPropertiesRecursive(
            JsonNode node, Schema schema) {
        Schema superTypeSchema = reflectionHelper.getSuperSchema(node, schema
                , true);
        ...
        rtn.addAll(getSuperTypeConstructorPropertiesRecursive(superSchemaNode
                , superTypeSchema));
    ...
    private void generateFieldsBuilderConstructor(
        ...
        JFieldVar instanceField =
                reflectionHelper.searchClassAndSuperClassesForField("instance"
                        , builderClass);
        ...
        builderConstructor.annotate(SuppressWarnings.class).param("value",
                "unchecked");
        ...
        // it
        constructorBlock.directStatement("// Skip initialization when called " +
                "from subclass");
    ...
    private void generateFieldsConcreteBuilderConstructor(
            JDefinedClass baseBuilderClass, JDefinedClass builderClass,
            JMethod instanceConstructor) {
        ...
        if (!(baseBuilderClass.isPrimitive()
                ...
                || Objects.equals(baseBuilderClass.fullName(), "java.lang" +
                ".Object"))) {
    ...
    private JMethod generateFieldsConstructor(
            JDefinedClass jclass, List<String> classProperties,
            List<String> combinedSuperProperties) {
        ...
        for (String property : combinedSuperProperties) {
            JFieldVar field =
                    reflectionHelper.searchSuperClassesForField(property,
                            jclass);
